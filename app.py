...
def collect_news() -> List[Dict]:
    ...
    try:
        for t, l in scrape_from("https://economictimes.indiatimes.com/markets", "a"):
    ...
    try:
        for t, l in scrape_from("https://www.moneycontrol.com/news/business/markets/", "a"):
            items.append({"title": t, "link": l})
    except Exception as e:
    ...
    # Mint (market)
    try:
        for t, l in scrape_from("https://www.livemint.com/market", "a"):
            items.append({"title": t, "link": l})
    except Exception as e:
...
def fetch_ipo_gmp():
    ...
    for tr in table.find_all("tr")[1:]:
        ...
        if is_sme and not ALLOW_SME:
            continue
        ipos.append({"name": name, "price": price, "dates": dates, "gmp": gmp})
    return ipos[:3]
...
# =========================
# Pre / Post market & FII/DII (best-effort)
# =========================
...
# =========================
# Flask (to keep Render web service happy)
# =========================
app = Flask(__name__)
...
# --- IMPORTANT: start scheduler even under gunicorn import ---
if os.getenv("RUN_SCHEDULER", "1") == "1":
    # Single worker in gunicorn → safe to start here
    start_scheduler()

# If you ever run this file directly (not needed on Render):
